...
from scripts.run_baselines import ensure_adalora_jobs, load_structured_config


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="批次訓練不同 Whisper 型號與兩種語言的 AdaLoRA 權重。"
    )
    parser.add_argument("--config", required=True, help="專案設定檔路徑。")
    parser.add_argument(
        "--baselines-config",
        default="configs/baselines.yaml",
        help="包含 adalora_jobs 的設定檔路徑。",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--redo",
        action="store_true",
        help="即使已存在權重也重新訓練全部 AdaLoRA 工作。",
    )
